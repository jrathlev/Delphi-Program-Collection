...
function ReadAltUserAccount (const NetResource : string) : boolean;

// Alternativen Benutzernamen setzen
function MakeAltUserAccount (const User,Pwd : string) : TUserAccount;
procedure SetAltUserAccount (const User,Pwd : string);
...
function ResetAltUserAccount (var NetName,Username : string) : integer; overload;
function ResetAltUserAccount : integer; overload;
...
begin
  with NetRes do begin
    dwScope:=RESOURCE_GLOBALNET; dwDisplayType:=RESOURCEDISPLAYTYPE_GENERIC;
    dwUsage:=RESOURCEUSAGE_CONNECTABLE; dwType:=RESOURCETYPE_DISK;
    lpProvider:=nil; lpLocalName:=nil; lpComment:=nil;
    if (copy(Path,1,2)='\\') then begin  // Netzwerkumgebung
      lpRemoteName:=StrNew(pchar(ExtractFileDrive(Path)));
  ...
  if ec<>NO_ERROR then begin
    ...
    if (length(AltUser.Username)>0) then begin
      ...
      repeat
        ...
        inc(nl);
        until (ec<>ERROR_NETNAME_DELETED) or (nl=3);  // try 3 times
      if ec=NO_ERROR then AltUserName:=AltUser.Username;
      end
    else if DirectoryExists(NetRes.lpRemoteName,false) then ec:=NO_ERROR
    else begin
      nl:=1024; pn:=StrAlloc(nl);
      ec:=WNetGetUser(PChar(NetRes.lpRemoteName),pn,nl);
      StrDispose(pn);
      if (ec=ERROR_NOT_CONNECTED) then begin  // als angemeldeter Benutzer versuchen
        if (length(StdUser)>0) then begin
          nl:=0;
          repeat
            if nl>0 then Sleep(1000); // wait 1 s
...
// Alternativen Benutzernamen löschen
function ResetAltUserAccount (var NetName,Username : string) : integer;
begin
  Username:=AltUsername; NetName:=RemoteName;
  if (length(AltUserName)=0) or (length(RemoteName)=0) then Result:=-1
  else begin
    Result:=WNetCancelConnection2(pchar(RemoteName),CONNECT_UPDATE_PROFILE,false);
    with AltUserAccount do begin
      Username:=''; Password:='';
      end;
    end;
...
function ResetAltUserAccount : integer;
var
  s,t : string;
begin
