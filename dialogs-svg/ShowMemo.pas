...
uses Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, Vcl.Graphics,
  ...
  Vcl.Printers, Vcl.ComCtrls, System.Math, ListSelectDlg, JrButtons,
...
type
  ...
  TShowDlgBtn = (sbOpen,sbErase,sbPrint,sbSearch,sbSection,sbError,sbFont,sbCodepage,sbExport);
  ...
  TShowTextDialog = class(TForm)
    ...
    SearchBtn: TJrButton;
    UpdateBtn: TJrButton;
    PrevErrBtn: TJrButton;
    PrevSectBtn: TJrButton;
    NextErrBtn: TJrButton;
    NextSectBtn: TJrButton;
    FontBtn: TJrButton;
    EncBtn: TJrButton;
    ExportBtn: TJrButton;
...
begin
  if FileExists(FIniName) then begin
    with TMemIniFile.Create(FIniName) do begin
      ...
      WriteString(ViewSect,IniExportPath,ExportPath);
...
procedure TShowTextDialog.FormAfterMonitorDpiChanged(Sender: TObject; OldDPI,
  NewDPI: Integer);
begin
...
begin
  pt:=GetCaretPos(Memo);
  StatusBar.Panels[0].Text:=SafeFormat(dgettext('dialogs-svg',' Line: %u of %u'),[pt.y+1,Memo.Lines.Count+1]);
  end;
...
begin
  ...
  //    if LineNr=Lines.Count then SelLength:=0 else SelLength:=Perform(EM_LINEINDEX,LineNr+1,0)-SelStart;
      StatusBar.Panels[0].Text:=SafeFormat(dgettext('dialogs-svg',' Line: %u of %u'),[LineNr+1,Lines.Count+1]);
      end;
    with FindDialog do Options:=Options -[frDown];
    end
  else with Memo do begin
    Clear;
    Lines.Add('*** '+SafeFormat(dgettext('dialogs-svg','Error loading file: "%s"'),[FName]));
    end;
  end;
...
//  if (Shift=[]) and (Key=VK_F3) then FindDialogFind(Sender);
  if (Key=VK_DELETE)  and (Memo.SelLength>0) then begin
    if ConfirmDialog (Point(Left+200,Top+100),
                      dgettext('dialogs-svg','Delete selected text?')) then begin
      with Memo do begin
        ReadOnly:=false;
        ClearSelection;
  ...
  begin
    with Printer,Canvas do begin
      ...
      if y=PrinterSettings.Margins.Top then begin   // Kopfzeile erzeugen
        font.style:=[fsbold];
        sf:=SafeFormat(dgettext('dialogs-svg','Page: %u'),[pagenumber]);
...
begin
  with SaveDialog do begin
    ...
    Filename:=AddPath(ExportPath,NewExt(ExtractFileName(TextName),'zip'));
    Title:=dgettext('dialogs-svg','Save text file as zip archive');
    DefaultExt:=ZipExt;
    Filter:=Format(dgettext('dialogs-svg','Zip archives')+'|*%s',[ZipExt]);
    if Execute then begin
      ExportPath:=ExtractFilePath(Filename);
      with TZipWrite.Create(Filename,ExtractFilePath(TextName)) do begin
        AddFile(TextName);
        WriteDirectory;
        Free;
        end;
...
begin
  if Memo.SelLength>0 then begin
    if ConfirmDialog (Point(Left+200,Top+100),dgettext('dialogs-svg','Delete selected text?')) then begin
      with Memo do begin
        ReadOnly:=false;
        ClearSelection;
    ...
    if ConfirmDialog (Point(Left+200,Top+100),SafeFormat(dgettext('dialogs-svg','Delete file "%s"?'),[TextName])) then begin
      DeleteFile(TextName);
      Memo.Clear;
      end;
...
begin
  ...
  if not FileExists(TextName) then Exit;
  fs:= TFileStream.Create(TextName,fmOpenRead or fmShareDenyNone);
  try
...
begin
  if FileExists(AIniName) then begin
    ...
    with TMemIniFile.Create(FIniName) do begin
      ...
      LWidth:=Width;
      ExportPath:=ReadString(ViewSect,IniExportPath,ExportPath);
      with Memo.Font do begin
...
begin
  if Visible then begin
    ...
    with ExportBtn do if sbExport in Buttons then begin
      Visible:=true; Left:=x; x:=x+Width-1;
      end
    else Visible:=false;
    ...
    if sbCodePage in Buttons then begin
      if CodePageList.Count=0 then GetCodePageList(CodePageList,dgettext('dialogs-svg','Automatic'));
      end;
    ...
    else ErrorDialog (dgettext('dialogs-svg','File: ')+SafeFormat(dgettext('dialogs-svg','"%s" not found!'),[TextDatei]));
    end;
  end;
