...
uses Winapi.Windows, System.SysUtils, System.Classes, Vcl.Graphics, Vcl.Forms,
  ...
  SVGIconImageListBase, SVGIconImageList, SVGIconImageCollection, JrButtons,
...
type
  TShellDirDialog = class(TForm)
    ...
    SVGImgColl: TSVGIconImageCollection;
    SVGImgLst24: TSVGIconVirtualImageList;
    SVGImgLst48: TSVGIconVirtualImageList;
    btbOK: TJrButton;
    btbCancel: TJrButton;
    spbNetwork: TJrSpeedButton;
    spbComputer: TJrSpeedButton;
    spbDesktop: TJrSpeedButton;
    spbMyFiles: TJrSpeedButton;
    spbHome: TJrSpeedButton;
    spbUp: TJrSpeedButton;
    spbNew: TJrSpeedButton;
...
begin
  ...
  with spbHome do begin
    Visible:=FDefaultDir<>'';
    Hint:=dgettext('dialogs-svg','Default: ')+FDefaultDir;
    end;
...
procedure TShellDirDialog.FormAfterMonitorDpiChanged(Sender: TObject; OldDPI,
  NewDPI: Integer);
begin
  SVGImgLst24.DPIChanged(Sender,OldDPI,NewDPI);
  SVGImgLst48.DPIChanged(Sender,OldDPI,NewDPI);
  end;
...
begin
  if not SetCurrentDir(FDefaultDir) then begin
    ErrorDialog(SafeFormat(dgettext('dialogs-svg','Directory not found:'+sLineBreak+'%s!'),[FDefaultDir]));
...
begin
  s:='';
  if InputQuery (ShellTreeView.Path,dgettext('dialogs-svg','New subdirectory:'),s) then begin
    s:=IncludeTrailingPathDelimiter(ShellTreeView.Path)+s;
    if not ForceDirectories(s) then
      ErrorDialog(SafeFormat(dgettext('dialogs-svg','Could not create directory:'+sLineBreak+'%s!'),[s]))
    else with ShellTreeView do begin
      Root:='rfMyComputer';
      spbComputer.Down:=true;
...
begin
  ...
  n:=SelectOption(dgettext('dialogs-svg','Delete directory'),
    SafeFormat(dgettext('dialogs-svg','Delete "%s"'),[s]),mtConfirmation,[fsBold],
    [dgettext('dialogs-svg','Definitely'),dgettext('dialogs-svg','To recycle bin')]);
  if n>=0 then begin
    spbUpClick(Sender);
    if n=0 then begin
      fc:=0; dc:=0; ec:=0;
      ...
      err:=ec>0;
      if not err then InfoDialog(SafeFormat(dgettext('dialogs-svg','%u directories with %u files deleted!'),[dc,fc]));
      end
    else begin
      ...
      if not err then InfoDialog(SafeFormat(dgettext('dialogs-svg','%s moved to Recycle Bin!'),[s]));
      end;
    if err then ErrorDialog(SafeFormat(dgettext('dialogs-svg','Error deleting directory:'+sLineBreak+'%s!'),[s]));
...
procedure TShellDirDialog.FormKeyDown(Sender: TObject; var Key: Word;
  ...
  procedure SpeedButtonClick (AButton : TJrSpeedButton);
  begin
...
begin
  if GetDiskFreeSpaceEx(pchar(IncludeTrailingPathDelimiter(APath)),na,nf,nil) then
    Result:=SizeToStr(na)+dgettext('dialogs-svg',' free of ')+SizeToStr(nf)
  else Result:='';
  end;
