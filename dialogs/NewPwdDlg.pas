...
function InputNewPassword (APos : TPoint; const Title : string; var Pwd : AnsiString;
                           Confirm : boolean; Show : boolean = false;
                           MinLen : integer = defMinPwdLength) : boolean; overload;
...
// Unicode password
function InputNewPassword (APos : TPoint; const Title : string; var Pwd : String;
                           Confirm : boolean; Show : boolean = false;
                           MinLen : integer = defMinPwdLength) : boolean; overload;
...
function GetPassword (APos : TPoint; const Title : string; var Pwd : string) : boolean; overload;
function GetPassword (APos : TPoint; const Title : string; var Pwd : AnsiString) : boolean; overload;
...
uses GnuGetText, ExtSysUtils, WinUtils, PasswordStrength,
...
{$IFDEF HDPI}   // scale glyphs and images for High DPI
procedure TNewPwdDialog.AfterConstruction;
begin
  inherited;
...
begin
  ...
  if not CanClose then begin
    if length(edtPwd.Text)<FMinLen then begin
      ErrorDialog(TryFormat(dgettext('dialogs','Minimum length of password is %u characters'),[FMinLen]));
      edtPwd.SetFocus;
      end
    else begin
      ErrorDialog(dgettext('dialogs','Passwords do not match!'));
      edtRepPwd.SetFocus;
      end;
    end;
  end;
...
function GetPassword (APos : TPoint; const Title : string; var Pwd : String) : boolean;
begin
  if not assigned(NewPwdDialog) then NewPwdDialog:=TNewPwdDialog.create(Application);
...
function GetPassword (APos : TPoint; const Title : string; var Pwd : AnsiString) : boolean;
begin
  if not assigned(NewPwdDialog) then NewPwdDialog:=TNewPwdDialog.create(Application);
  Result:=NewPwdDialog.Execute(APos,Title,Pwd,false);
  FreeAndNil(NewPwdDialog);
